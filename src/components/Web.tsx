...
const buildRadial = (labels: string[], radius: number) => {
  ...
  const braids: [number, number][] = labels.map((_, i) => [
    degToRadians(angle) * i,
    radius,
  ]);
...
export const Web = ({ axis, ticks = 7, labels, center, radial }: Props) => {
      ...
      {labels.map((label, i) => {
        ...
        return (
          <g key={id}>
            ...
            <text x={center[0] + c[0]} y={center[1] + c[1]}>
              {label}
            </text>
          </g>
        );
