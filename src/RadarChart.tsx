...
const buildArea = (
  ...
  const points: [number, number][] = datum.axes
    .map((axis) => {
...
export const RadarChart = ({
  ...
  return (
    <Base height={height} id={id} title={title} width={width}>
      <Web labels={labels} center={center} />
      {data.map((datum, i) => {
            ...
            {pointsData[i].map((ptns, j) => {
              ...
              return (
                <Point
                  ...
                  z={pointStyle.z ?? 3}
                >
                  {pointStyle?.showTitle && (
                    <title>
                      {labelId}: {datum.axes[j].value}
                    </title>
                  )}
                </Point>
              );
